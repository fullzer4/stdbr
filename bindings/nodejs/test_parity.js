...
describe("RG", () => {
  describe("parse", () => {
    for (const c of golden.rg.parse) {
      it(`${c.input} (${c.uf})`, () => {
        ...
        strictEqual(rg.checkDigit ?? null, c.check_digit);
      });
    }
  });
  ...
  describe("format", () => {
    for (const c of golden.rg.format) {
      it(`${c.input || "(empty)"} (${c.uf})`, () => {
        strictEqual(rgFormat(c.input, stateFromAbbreviation(c.uf)) ?? null, c.expected);
      });
    }
  });
  ...
  describe("remove_symbols", () => {
    for (const c of golden.rg.remove_symbols) {
      it(`${c.input} (${c.uf})`, () => {
        strictEqual(rgRemoveSymbols(c.input, stateFromAbbreviation(c.uf)), c.expected);
      });
    }
  });
  ...
  describe("compute_check_digit", () => {
    for (const c of golden.rg.compute_check_digit) {
      it(`${c.base} (${c.uf})`, () => {
        const result = rgComputeCheckDigit(c.base, stateFromAbbreviation(c.uf));
        strictEqual(result ?? null, c.expected);
      });
    }
  });
