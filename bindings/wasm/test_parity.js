  ...
  describe("parse", () => {
    for (const c of golden.cep.parse) {
      it(c.input, () => {
        ...
        const abbr = cep.state !== undefined ? stateAbbreviation(cep.state) : null;
  ...
  describe("from_abbreviation", () => {
    for (const c of golden.uf.from_abbreviation) {
      it(c.input || "(empty)", () => {
        ...
        strictEqual(s !== undefined ? stateAbbreviation(s) : null, c.expected);
      });
    }
  });
});
...
describe("Municipio", () => {
  ...
  describe("search_by_name", () => {
    for (const s of golden.municipio.search_by_name) {
      it(s.query, () => {
        ...
        strictEqual(results.count, s.results.length);
        for (let i = 0; i < s.results.length; i++) {
          const m = results.get(i);
          strictEqual(m.ibgeCode, s.results[i].ibge_code);
          strictEqual(m.name, s.results[i].name);
        }
      });
    }
  });
  ...
  describe("by_state_count", () => {
    for (const c of golden.municipio.by_state_count) {
      it(c.state, () => {
        strictEqual(municipiosByState(stateFromAbbreviation(c.state)).count, c.count);
      });
    }
  });
});
